...
type RestEventModel struct {
	...
	Zones          []*RestZoneModel `xml:"zone"`
}

func (f *RestEventModel) String() string {
	return fmt.Sprintf("{EventID:%d EventStartDate:%+v EventEndDate:%+v SellFrom:%+v SellTo:%+v SoldOut:%v Zones:%+v}",
		f.EventID, f.EventStartDate, f.EventEndDate, f.SellFrom, f.SellTo, f.SoldOut, f.Zones)
}
...
func RestBaseEventModelToAppEvents(restModel []*RestBaseEventModel) []*events.Event {
	...
	for _, restBaseEvent := range restModel {
		...
		for _, restEvent := range restBaseEvent.Events {
			...
			for _, restZone := range restEvent.Zones {
